...
RF models used for ``supp_point3_rf_multihorizon_model_comparison``.
...
PHASES = {
    ...
    "Longer-term (7-9 months ahead)": (7, 8, 9),
}
COLORS = {
...
def summarize() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for horizon in range(1, 10):
        path = TABLES / f"supp_point3_rf_h{horizon}_shap_state_month_class.csv"
...
def plot(phase_summary: pd.DataFrame, legend: bool) -> None:
    ...
    for axis, (phase, _) in zip(axes, PHASES.items()):
        ...
        for y, region in enumerate(REGIONS):
            ...
            for domain in DOMAINS:
                ...
                bar = axis.barh(y, value, left=left, height=0.64, color=COLORS[domain], edgecolor="white", linewidth=0.45)
    ...
    stem = "supp_point3_rf_regional_class_contribution_by_horizon"
    if not legend:
        stem += "_wo_legend"
...
def main() -> None:
    ...
    by_horizon.to_csv(TABLES / "supp_point3_rf_regional_class_net_shap_by_horizon.csv", index=False)
    phase_summary.to_csv(TABLES / "supp_point3_rf_regional_class_net_shap_by_phase.csv", index=False)
