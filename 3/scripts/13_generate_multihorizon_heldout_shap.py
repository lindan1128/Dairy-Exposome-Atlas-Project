#!/usr/bin/env python3
"""Generate held-out feature SHAP for one completed Point 3 forecast horizon.
...
def state_month_feature_shap(
    ...
    out = pd.DataFrame({
        ...
        "feature_value": test[exposure_features].to_numpy(float).ravel(),
...
def main() -> int:
    ...
    configs = pd.read_csv(TAB / "point3_point4aligned_multihorizon_configs_by_year.csv")
    selected = pd.read_csv(TAB / "point3_point4aligned_multihorizon_selected_features_by_year.csv")
    stored_predictions = pd.read_csv(TAB / "point3_point4aligned_multihorizon_predictions.csv")
    configs = configs.loc[
        configs.horizon_months.eq(horizon)
        & configs.model.eq("history_exposure_selected_hgb_nested")
    ...
    for config in configs.itertuples(index=False):
        ...
        model.fit(train[[*history, *features]], train._target_loss, sample_weight=train.milk_cows_head)
        predicted_loss = model.predict(test[[*history, *features]])
        ...
        reference = stored_predictions.loc[
            ...
            [*keys, "history_exposure_selected_hgb_predicted_loss_pct"],
        ]
        ...
        if checked.history_exposure_selected_hgb_predicted_loss_pct.isna().any():
            raise RuntimeError(f"Missing stored prediction for h{horizon}, {test_year}.")
        max_abs_difference = float(np.max(np.abs(
            checked.recomputed_loss - checked.history_exposure_selected_hgb_predicted_loss_pct
        )))
        if not np.allclose(
            checked.recomputed_loss,
            checked.history_exposure_selected_hgb_predicted_loss_pct,
            rtol=0.0,
            atol=1e-12,
        ):
            raise RuntimeError(
        ...
        rows.append(state_feature_shap(model, test, all_features, exposure_meta, test_year))
        domain_rows.append(state_month_class_shap(model, test, all_features, exposure_meta, test_year, horizon))
        if args.write_state_month_feature:
            state_month_feature_rows.append(
                state_month_feature_shap(model, test, all_features, exposure_meta, test_year, horizon)
            )
        print(f"computed held-out H{horizon} SHAP for {test_year}", flush=True)

    raw = pd.concat(rows, ignore_index=True)
    pd.DataFrame(prediction_checks).to_csv(
        TAB / f"point3_point4aligned_h{horizon}_shap_prediction_reproducibility_check.csv", index=False
    )
    raw.to_csv(TAB / f"point3_point4aligned_h{horizon}_shap_state_feature_by_year.csv", index=False)
    pd.concat(domain_rows, ignore_index=True).to_csv(
        TAB / f"point3_point4aligned_h{horizon}_shap_state_month_class.csv", index=False
    )
    if args.write_state_month_feature:
        pd.concat(state_month_feature_rows, ignore_index=True).to_csv(
            TAB / f"point3_point4aligned_h{horizon}_shap_state_month_feature.csv", index=False
        )
    raw.groupby(
    ...
    ).to_csv(TAB / f"point3_point4aligned_h{horizon}_shap_state_feature_summary.csv", index=False)
    print(f"wrote H{horizon} held-out SHAP for {configs.test_year.nunique()} outer test years")
    return 0
