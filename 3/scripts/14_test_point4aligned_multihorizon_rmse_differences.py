...
TABLES = HERE / "tables"
INPUT = TABLES / "point3_point4aligned_multihorizon_performance_by_year_scope.csv"
OUTPUT = TABLES / "point3_point4aligned_multihorizon_paired_rmse_tests.csv"
LB_TO_KG = 0.45359237
...
def main() -> None:
    ...
    expected = {"history_hgb_nested", "history_exposure_selected_hgb_nested"}
    ...
    for horizon, data in annual.groupby("horizon_months", sort=True):
        history = data.history_hgb_nested.to_numpy(float)
        exposome = data.history_exposure_selected_hgb_nested.to_numpy(float)
