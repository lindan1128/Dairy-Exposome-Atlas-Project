#!/usr/bin/env python3
"""Nested rolling-origin 1-12 month Random Forest forecasts using Point 2's exposures.

Each horizon is a separate forecasting task. Within every outer forecast year,
all feature screening and Random Forest tuning occur only in earlier rolling validation
...
LB_TO_KG = 0.45359237
K_GRID = (25, 75, 150)
# A compact, regularized RF grid keeps tuning proportional to the HGB workflow
# while varying the two parameters most consequential for this small panel:
# feature subsampling and terminal-node size.
MODEL_GRID = (
    {"n_estimators": 200, "max_features": 0.50, "min_samples_leaf": 5, "max_depth": None,
     "bootstrap": True, "max_samples": 0.75, "n_jobs": -1},
    {"n_estimators": 200, "max_features": 0.80, "min_samples_leaf": 10, "max_depth": None,
     "bootstrap": True, "max_samples": 0.75, "n_jobs": -1},
)
...
def fit_predict(
    train: pd.DataFrame, test: pd.DataFrame, features: list[str], params: dict
) -> tuple[RandomForestRegressor, SimpleImputer, np.ndarray]:
    """Fit a fold-local imputer and RF; no validation/test values inform either."""
    imputer = SimpleImputer(strategy="median", keep_empty_features=True)
    train_x = imputer.fit_transform(train[features])
    test_x = imputer.transform(test[features])
    model = RandomForestRegressor(**params, random_state=RANDOM_STATE)
    model.fit(
        train_x, train._target_loss.to_numpy(float),
        sample_weight=train.milk_cows_head.to_numpy(float),
    )
    return model, imputer, model.predict(test_x)


def rank_exposures(train: pd.DataFrame, history: list[str], exposures: list[str]) -> list[str]:
    """Training-only residual screen used solely to choose the RF input set."""
...
def precompute_inner_cv(
    ...
    for validation_year in validation_years:
        ...
        for candidate_id, params in enumerate(MODEL_GRID):
            _, _, prediction = fit_predict(train, valid, history, params)
            rows.append({
                "horizon_months": horizon, "inner_validation_year": validation_year,
                "model": "history_rf_nested", "candidate_id": candidate_id,
        ...
        for k in K_GRID:
            selected = ranking[:k]
            for candidate_id, params in enumerate(MODEL_GRID):
                _, _, prediction = fit_predict(train, valid, [*history, *selected], params)
                rows.append({
                    "horizon_months": horizon, "inner_validation_year": validation_year,
                    "model": "history_exposure_selected_rf_nested", "candidate_id": candidate_id,
...
def state_feature_shap(
    model: RandomForestRegressor, imputer: SimpleImputer, test: pd.DataFrame, features: list[str], exposure_meta: pd.DataFrame, test_year: int
) -> pd.DataFrame:
...
def make_figure(performance: pd.DataFrame, show_legend: bool) -> None:
    ...
    labels = {
        "history_rf_nested": "Baseline forecast",
        "history_exposure_selected_rf_nested": "Exposome-enhanced forecast",
    }
    colors = {
        "history_rf_nested": "#B5C99F",
        "history_exposure_selected_rf_nested": "#FFCC88",
    }
    figure_height = 3.9 if show_legend else 3.45
    ...
    history = model_data["history_rf_nested"]
    exposed = model_data["history_exposure_selected_rf_nested"]
    for model in ("history_rf_nested", "history_exposure_selected_rf_nested"):
        data = model_data[model]
        mean = smooth(data, "mean_rmse")
        lower = smooth(data.assign(bound=data.mean_rmse - data.ci_half_width), "bound")
        upper = smooth(data.assign(bound=data.mean_rmse + data.ci_half_width), "bound")
        ax.fill_between(
            x_smooth,
            lower,
            upper,
    ...
    annotation_style = dict(
        arrowprops=dict(arrowstyle="->", color="#222222", linewidth=0.85, shrinkA=2, shrinkB=2),
        fontsize=9,
        color="#222222",
        ha="left",
        va="center",
    ...
    ax.annotate(
        "Baseline forecast",
        xy=(6.0, float(smooth(history, "mean_rmse", np.array([6.0]))[0])),
        xytext=(7.15, 25.8),
        **annotation_style,
    )
    ax.annotate(
        "Exposome-enhanced forecast",
        xy=(6.0, float(smooth(exposed, "mean_rmse", np.array([6.0]))[0])),
        xytext=(7.15, 17.4),
        **annotation_style,
    )
    ax.set_xlim(1, 12)
    ...
    output = FIG / f"supp_point3_rf_multihorizon_model_comparison{suffix}.svg"
...
def main() -> int:
    ...
    for horizon in horizons:
        ...
        for outer_year in test_years:
            ...
            hist_id, _, hist_params, hist_years = choose_outer_config(
                inner_cv, outer_year, "history_rf_nested", args.max_inner_folds
            )
            expo_id, winner_k, expo_params, expo_years = choose_outer_config(
                inner_cv, outer_year, "history_exposure_selected_rf_nested", args.max_inner_folds
            )
            ...
            hist_model, hist_imputer, hist_prediction = fit_predict(outer_train, outer_test, history, hist_params)
            expo_model, expo_imputer, expo_prediction = fit_predict(outer_train, outer_test, [*history, *selected], expo_params)
            ...
            prediction["history_rf_predicted_loss_pct"] = hist_prediction
            prediction["history_rf_predicted_milk_lb_per_cow"] = loss_to_milk(prediction.milk_per_cow_lb, hist_prediction)
            prediction["history_exposure_selected_rf_predicted_loss_pct"] = expo_prediction
            prediction["history_exposure_selected_rf_predicted_milk_lb_per_cow"] = loss_to_milk(prediction.milk_per_cow_lb, expo_prediction)
            prediction_rows.extend(prediction.to_dict("records"))

            for model_name, predicted in (
                ("history_rf_nested", hist_prediction),
                ("history_exposure_selected_rf_nested", expo_prediction),
            ):
            ...
            config_rows.extend([
                {"horizon_months": horizon, "test_year": outer_year, "model": "history_rf_nested", "candidate_id": hist_id, "k_exposure_features": 0, "params": json.dumps(hist_params, sort_keys=True), "inner_validation_years": ",".join(map(str, hist_years)), "inner_selection_metric": "mean cow-weighted target-month milk RMSE; MAE tie-breaker"},
                {"horizon_months": horizon, "test_year": outer_year, "model": "history_exposure_selected_rf_nested", "candidate_id": expo_id, "k_exposure_features": winner_k, "params": json.dumps(expo_params, sort_keys=True), "inner_validation_years": ",".join(map(str, expo_years)), "inner_selection_metric": "mean cow-weighted target-month milk RMSE; MAE tie-breaker"},
            ])
            ...
            if horizon == 1 and not args.skip_shap:
                shap_rows.append(state_feature_shap(expo_model, expo_imputer, outer_test, [*history, *selected], exposure_meta, outer_year))
    ...
    baseline = overall.loc[overall.model.eq("history_rf_nested"), ["horizon_months", "target_milk_rmse_lb_per_cow_mean", "target_milk_mae_lb_per_cow_mean"]].rename(columns={
    ...
    pd.DataFrame(prediction_rows).to_csv(TAB / "supp_point3_rf_multihorizon_predictions.csv", index=False)
    performance.to_csv(TAB / "supp_point3_rf_multihorizon_performance_by_year_scope.csv", index=False)
    pd.DataFrame(inner_rows).to_csv(TAB / "supp_point3_rf_multihorizon_inner_cv_results.csv", index=False)
    pd.DataFrame(config_rows).to_csv(TAB / "supp_point3_rf_multihorizon_configs_by_year.csv", index=False)
    pd.DataFrame(selected_rows).to_csv(TAB / "supp_point3_rf_multihorizon_selected_features_by_year.csv", index=False)
    overall.to_csv(TAB / "supp_point3_rf_multihorizon_overall_summary.csv", index=False)
    if shap_rows:
        shap_by_year = pd.concat(shap_rows, ignore_index=True)
        shap_by_year.to_csv(TAB / "supp_point3_rf_h1_shap_state_feature_by_year.csv", index=False)
        ...
        ).to_csv(TAB / "supp_point3_rf_h1_shap_state_feature_summary.csv", index=False)
    manifest = {
        "method": "Separate nested rolling-origin continuous Random Forest forecasts for 1-12 month horizons, with training-only history-adjusted residual screening of Point 2-aligned exposure features and fold-local median imputation.",
        ...
        "shap": "Held-out H1 selected-RF feature SHAP only; positive values increase predicted one-month milk loss.",
    }
    (LOG / "supp_point3_rf_multihorizon_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
