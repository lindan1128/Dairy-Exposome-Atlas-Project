...
perf_observed <- read_csv(file.path(tab, "point6_region_best_forecast_by_state_performance.csv"), show_col_types = FALSE) %>%
  mutate(imputed = FALSE)

missing_states <- analysis_states %>%
  filter(!state_alpha %in% perf_observed$state_alpha)

region_means <- perf_observed %>%
  group_by(region) %>%
  summarise(
    across(
      c(baseline_rmse, region_best_exposome_rmse, delta_rmse, improvement_pct),
      ~ mean(.x, na.rm = TRUE)
    ),
    n_state_months = NA_real_,
    n_test_years = NA_real_,
    region_best_subset_id = first(region_best_subset_id),
    region_best_blocks = first(region_best_blocks),
    .groups = "drop"
  )

perf_imputed <- missing_states %>%
  left_join(region_means, by = "region") %>%
  mutate(imputed = TRUE)

perf_all <- bind_rows(perf_observed, perf_imputed)
write_csv(perf_all, file.path(tab, "point6_region_best_forecast_by_state_performance_imputed26.csv"))
...
base <- file.path(fig, "point6_region_best_forecast_by_state_rmse_imputed26")
