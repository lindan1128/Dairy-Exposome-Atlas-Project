...
TAB = POINT / "tables"
IN = TAB / "point1_chord_signal_yearly_class_summary.csv"
...
SCOPES = {"per_cow_50": "Milk per cow", "total_same_50": "Total production"}
    ...
    for scope, label in SCOPES.items():
        for dom in DOMAINS:
            ...
            for i in range(len(pts)):
                xx = xs[:i] + xs[i + 1:]
                yy = ys[:i] + ys[i + 1:]
                ai, bi, r2i, _ = ols(xx, yy)
                loyo.append((xs[i], ai, bi, r2i))
                lines.append({
                    "phenotype": label, "domain": dom, "dropped_year": xs[i],
                    "slope": bi, "intercept": ai,
                })
