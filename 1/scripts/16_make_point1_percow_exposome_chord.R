...
# Class arcs are grouped and coloured by domain; ribbons encode effect
...
d_all <- read_csv(file.path(tab, "point1_native_only_endpoint_exwas_associations.csv"), show_col_types = FALSE) %>%
  rename(domain = class, source_class = source_domain) %>%
  mutate(
    domain = case_when(
      domain == "Pandemic shock" & mechanistic_subclass_en == "COVID" ~ "COVID",
      domain == "Pandemic shock" & mechanistic_subclass_en == "HPAI" ~ "HPAI",
...
build_data <- function(scope) {
  ...
  if (scope == "per_cow_50" && nrow(pollution_direction_ok) > 0) {
    d <- d %>%
...
class_arc_data <- function(side = c("left", "right"), scale = 0.96, gap = 0, rr = 1.25, n = 44) {
  side <- match.arg(side)
  agg <- d_all %>%
    filter(phenotype_scope == "per_cow_50") %>%
...
mk_paired <- function() {
  left <- build_data("total_same_50")
  right <- build_data("per_cow_50")
  circle_scale <- 0.96
  rb <- bind_rows(
...
mk_paired_signal_only <- function(show_endpoint_titles = TRUE) {
  left <- build_signal_only_data("total_same_50")
  right <- build_signal_only_data("per_cow_50")
  circle_scale <- 0.96
  half_gap <- 0.075
  rb <- bind_rows(
...
outp <- file.path(fig, "main_point1_total50_percow_paired_exposome_chord.svg")
ggsave(outp, pp, width = 9, height = 4.7, units = "in", bg = "transparent"); clean(outp)
ppw <- mk_paired() + theme(plot.title = element_blank())
outpw <- file.path(fig, "main_point1_total50_percow_paired_exposome_chord_wo_legend.svg")
...
outs <- file.path(fig, "main_point1_total50_percow_paired_exposome_chord_signal_only.svg")
...
outsw <- file.path(fig, "main_point1_total50_percow_paired_exposome_chord_signal_only_wo_legend.svg")
