...
def main() -> int:
    ...
    if not rows:
        if OUT.exists():
            ...
            ok = (existing["state_alpha"].nunique() == 50 and existing["year"].min() <= 2000 and existing["year"].max() >= 2025)
            if ok:
                print(f"raw county Daymet unavailable at {RAW}; retained existing 50-state unweighted forms {OUT} rows={len(existing):,}", flush=True)
                return 0
