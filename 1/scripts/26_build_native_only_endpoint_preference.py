...
OUT_CLASS = TAB / "point1_native_only_domain_class_endpoint_preference.csv"
OUT_DOMAIN = TAB / "point1_native_only_domain_endpoint_summary.csv"
CLEAN_DICT = (
    ROOT
    / "data"
    / "us_expose_new"
...
EXCLUDE_EXPOSURE_CLASSES = {
    # Milk-cow inventory and directly derived dairy-scale variables overlap
    # with the observation weights and total-production definitions.
    "Dairy scale",
...
PHENO_ORDER = ["milk_per_cow_lb", "milk_production_lb_same50", "milk_production_lb"]
PHENO_LABEL = {
    "milk_per_cow_lb": "per_cow_50",
    "milk_production_lb_same50": "total_same_50",
    "milk_production_lb": "total_50",
}
PHENOTYPE_LONG_LABEL = {
    "milk_per_cow_lb": ("per_cow_50", "Milk per cow: 50 states"),
    "milk_production_lb_same50": ("total_same_50", "Total production: same 50 states"),
    "milk_production_lb": ("total_50", "Total production: 50 states"),
}


def load_panel_for_missing_exposures() -> pd.DataFrame:
    panel = L.load_panel()
    # Same-50 total-production endpoint paired with the per-cow endpoint; in the
    # 50-state rerun it contains monthly-equivalent total production for the
    # same 50 states as the derived per-cow outcome.
...
def apply_clean_dictionary_metadata(df: pd.DataFrame, clean_dict: pd.DataFrame) -> pd.DataFrame:
    ...
    meta = meta.rename(
        columns={
            ...
            "Subdomain": "_clean_subclass",
            "mechanistic_domain_en": "_clean_mechanistic_subclass_en",
            "mechanistic_domain_ch": "_clean_mechanistic_subclass_zh",
    ...
    direct_map = {
        ...
        "mechanistic_domain_en": "_clean_mechanistic_subclass_en",
        "mechanistic_domain_zh": "_clean_mechanistic_subclass_zh",
...
def normalize_association_schema(df: pd.DataFrame) -> pd.DataFrame:
    """Accept either hierarchy schema, then use the legacy internal names below."""

    return df.rename(
        columns={
            "class_zh": "domain_zh",
            "class": "domain",
            "mechanistic_subclass_zh": "mechanistic_domain_zh",
            "mechanistic_subclass_en": "mechanistic_domain_en",
            "class_assoc": "domain_assoc",
            "class_matched_model": "domain_matched_model",
            "class_matched_controls": "domain_matched_controls",
            "class_matched_recomputed": "domain_matched_recomputed",
            "class_matched_direction_stable": "domain_matched_direction_stable",
            "source_domain": "source_class",
        }
    )


def publish_association_schema(df: pd.DataFrame) -> pd.DataFrame:
    """Write outputs with the corrected domain -> class -> subclass hierarchy."""

    return df.rename(
        columns={
            "domain_zh": "class_zh",
            "domain": "class",
            "mechanistic_domain_zh": "mechanistic_subclass_zh",
            "mechanistic_domain_en": "mechanistic_subclass_en",
            "domain_assoc": "class_assoc",
            "domain_matched_model": "class_matched_model",
            "domain_matched_controls": "class_matched_controls",
            "domain_matched_recomputed": "class_matched_recomputed",
            "domain_matched_direction_stable": "class_matched_direction_stable",
...
def append_missing_sparse_event_rows(df: pd.DataFrame, clean_dict: pd.DataFrame, missing_vars: list[str]) -> pd.DataFrame:
    """Add clean sparse-event variables that were absent from the legacy association source."""
...
def load_clean_dictionary() -> pd.DataFrame:
    ...
    if "subclass" in d.columns:
        d = d.rename(
            columns={
                "domain": "source_domain",
                "domain_ch": "source_domain_ch",
                "class": "domain",
                "class_ch": "domain_ch",
                "subclass": "Subdomain",
                "mechanistic_subclass_en": "mechanistic_domain_en",
                "mechanistic_subclass_ch": "mechanistic_domain_ch",
            }
        )
    d = d[d["used_in_exwas"].fillna(False).astype(bool)].copy()
    d = d[~d["domain"].isin(EXCLUDE_EXPOSURE_CLASSES)].copy()
...
def load_source_associations() -> tuple[pd.DataFrame, Path]:
    for path in [ASSOC, PRECLEAN_ASSOC, OUT_ASSOC]:
        if path.exists():
            return normalize_association_schema(pd.read_csv(path, low_memory=False)), path
...
def endpoint_pattern(row: pd.Series) -> str:
    pc = row.get("n_by_per_cow_50", 0), row.get("n_p01_per_cow_50", 0), row.get("max_r2_per_cow_50", 0)
    t26 = row.get("n_by_total_same_50", 0), row.get("n_p01_total_same_50", 0), row.get("max_r2_total_same_50", 0)
...
def main() -> int:
    ...
    df = df[~df["domain"].isin(EXCLUDE_EXPOSURE_CLASSES)].copy()
    ...
    df = df[~df["domain"].isin(EXCLUDE_EXPOSURE_CLASSES)].copy()
    df = df[df["phenotype"].isin(PHENO_ORDER)].copy()
    df["phenotype_scope"] = df["phenotype"].map(PHENO_LABEL)
    df["phenotype_label"] = df["phenotype"].map({k: v[1] for k, v in PHENOTYPE_LONG_LABEL.items()})
    ...
    publish_association_schema(native).to_csv(OUT_ASSOC, index=False, encoding="utf-8-sig")

    domain_summary = summarize(native, ["source_class", "phenotype"])
    domain_summary["phenotype_scope"] = domain_summary["phenotype"].map(PHENO_LABEL)
    domain_summary["domain_order"] = domain_summary["source_class"].map({x: i for i, x in enumerate(CLASS_ORDER)})
    domain_summary["phenotype_order"] = domain_summary["phenotype"].map({x: i for i, x in enumerate(PHENO_ORDER)})
    domain_summary = domain_summary.sort_values(["domain_order", "phenotype_order"]).drop(
        columns=["domain_order", "phenotype_order"]
    )
    domain_summary.rename(columns={"source_class": "domain"}).to_csv(OUT_DOMAIN, index=False, encoding="utf-8-sig")

    class_summary = summarize(native, ["source_class", "domain", "phenotype"])
    class_summary["phenotype_scope"] = class_summary["phenotype"].map(PHENO_LABEL)
    wide = class_summary.pivot(
    ...
    wide.rename(columns={"source_class": "domain", "domain": "class"}).to_csv(OUT_CLASS, index=False, encoding="utf-8-sig")
