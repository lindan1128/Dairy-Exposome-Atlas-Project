...
d <- read_csv(file.path(tab, "point1_chord_signal_yearly_class_summary.csv"), show_col_types = FALSE) %>%
  ...
  mutate(
    ...
    phenotype_scope = factor(phenotype_scope, levels = c("total_same_50", "per_cow_50"),
                             labels = endpoint_labels[c("total_same_50", "per_cow_50")]),
...
write_csv(trend_summary, file.path(tab, "point1_chord_signal_yearly_class_trend_summary.csv"))
...
trend_labels <- d %>%
  ...
  mutate(
    ...
    label = paste0(ifelse(stars == "", "", paste0(stars, " ")), "R²=", sprintf("%.2f", r2)),
    x = 2024.3,
    y = Inf,
    hjust_label = 1,
...
p <- ggplot(d, aes(year_plot, median_abs_beta, color = domain, group = domain)) +
  ...
  labs(
    title = "Year-specific association strength by class and endpoint",
    x = NULL,
