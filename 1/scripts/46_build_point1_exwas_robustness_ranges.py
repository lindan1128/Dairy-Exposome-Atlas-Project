...
OUT_LOSO_DM = TAB / "point1_exwas_class_matched_loso_beta_robustness.csv"
...
EVENT_OUTCOMES = {
    "total_same_50": "milk_production_from_cows_million_kg",
    "per_cow_50": "milk_per_cow_kg",
}
...
def build_event_study_sensitivity(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for endpoint, outcome in EVENT_OUTCOMES.items():
        phenotype_label = "Milk per cow" if endpoint == "per_cow_50" else "Total production"
...
def build_domain_matched_loso(panel: pd.DataFrame, assoc: pd.DataFrame) -> pd.DataFrame:
    if OLD_LOSO.exists():
        out = harmonize_assoc_columns(pd.read_csv(OLD_LOSO, low_memory=False))
        if "domain" in out.columns:
            out = out[~out["domain"].isin(EXCLUDED_DOMAINS)].copy()
    else:
