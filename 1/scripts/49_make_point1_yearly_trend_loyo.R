...
dom_labels <- c("Heat", "Cold", "Severe weather", "Forage", "Pesticides",
                "Feed market", "Dairy market", "Market demand")
...
pts <- read_csv(file.path(tab, "point1_chord_signal_yearly_class_summary.csv"),
                show_col_types = FALSE) %>%
  filter(phenotype_scope == "per_cow_50", domain %in% dom_levels) %>%
