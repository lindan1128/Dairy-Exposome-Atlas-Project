...
normalize_domain_names <- function(d) {
  if (!"domain" %in% names(d) && "class" %in% names(d)) {
    d <- d %>% rename(domain = class)
  }
  if (!"source_class" %in% names(d) && "source_domain" %in% names(d)) {
    d <- d %>% rename(source_class = source_domain)
  }
  if (!"mechanistic_domain_en" %in% names(d) && "mechanistic_subclass_en" %in% names(d)) {
    d <- d %>% rename(mechanistic_domain_en = mechanistic_subclass_en)
  }
  d %>%
    mutate(
      domain = recode(
...
assoc <- read_csv(file.path(tab, "point1_native_only_endpoint_exwas_associations.csv"), show_col_types = FALSE) %>%
  ...
  filter(
    window == "native",
    phenotype_scope %in% c("total_same_50", "per_cow_50"),
...
loso <- read_csv(file.path(tab, "point1_exwas_class_matched_loso_beta_robustness.csv"), show_col_types = FALSE) %>%
  normalize_domain_names() %>%
  keep_event_representatives() %>%
...
make_loso_endpoint <- function(scope, label, show_y = TRUE) {
  ...
  ggplot(d_loso, aes(y = y_order)) +
    ...
    scale_fill_manual(values = domain_pal, labels = domain_labels, name = "Class", drop = FALSE) +
    y_scale +
...
make_loso_domain_row <- function(dom, is_first = FALSE, is_last = FALSE) {
  ...
  d_domain <- loso %>%
    filter(
      phenotype_scope %in% c("per_cow_50", "total_same_50"),
      as.character(domain) == dom
    )
  xlim_domain <- d_domain %>%
    ...
    abs() %>%
    max(na.rm = TRUE)
  row_fun("per_cow_50", dom, if (is_first) "Milk per cow" else NULL, TRUE, is_last, xlim_domain) |
    row_fun("total_same_50", dom, if (is_first) "Total production" else NULL, FALSE, is_last, xlim_domain)
}
...
make_modelspec_domain_row <- function(dom, is_first = FALSE, is_last = FALSE) {
  ...
  row_fun(
    "per_cow_50", dom, if (is_first) "Milk per cow" else NULL, TRUE, is_last,
    show_legend = dom == "Heat",
    xlim_override = xlim_domain
  ) |
    row_fun(
      "total_same_50", dom, if (is_first) "Total production" else NULL, FALSE, is_last,
...
weighting <- read_csv(file.path(tab, "point1_exwas_weighting_robustness_beta.csv"), show_col_types = FALSE) %>%
  ...
  mutate(
    domain = factor(domain, levels = domain_levels),
    phenotype_label = recode(phenotype_scope, total_same_50 = "Total production", per_cow_50 = "Milk per cow"),
...
make_weight_panel <- function(analysis_name) {
  ...
  ggplot(d, aes(main_beta, sensitivity_beta, color = domain, shape = phenotype_label)) +
    ...
    scale_color_manual(values = domain_pal, labels = domain_labels, name = "Class", drop = TRUE) +
